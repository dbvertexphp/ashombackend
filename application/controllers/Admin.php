...
class Admin extends CI_Controller
{
  ...
  public function edit_users(){
        ...
        $id=$_POST['id1'];
          $expire_on = $this->input->post("expiry1");
	       $subscription_type = $this->input->post("subscription_type1");
         $where=array('day'=>$subscription_type);
         $sub =$this->admin_model->get_record_where('plans_capacity',$where);
         foreach($sub as $value){
          $subscription= $value->plan_type;
         }
        $idata["user_id"]= $id;
      	$idata["subscription_type"]=$subscription;
      	
        $idata["expire_on"] = $expire_on;
      	$idata["created"] = date("Y-m-d");
         
          $result = $this->db->update("subscription", $idata, ["user_id"=>$id]);
  ...
  public function Documents($country = "", $company_id = 0, $year = 0, $period = "", $flag = 0)
  {
    ...
    if ($session_id) {
      ...
      if (isset($_POST['upload'])) {
        ...
        $ac_company_name = str_replace(" ", "%20", $company_name);
        ...
        foreach ($_FILES['files']['name'] as $i => $name) {
          if (strlen($_FILES['files']['name'][$i]) > 1) {
            ...
            if (!in_array((object) array($year), $total_years_arr))
              ...
              $period = $quarter;
              $document_link = base_url()."uploads/documents/$country/$ac_company_name/$year/$period/" . $_FILES['files']['name'][$i];
        ...
        if ($uploadstatus) {
          ...
          for ($counta = 0; $counta < count($doc_data); $counta++)
            ...
            $company_symbol = $this->db->get_where("companies", ["id" => $company_id])->row()->SymbolTicker;
            $image = $this->db->get_where("companies", ["id" => $company_id])->row()->image;
            $devices = array();
            ...
            $dataji["type"] ='Financial Report';
            $dataji["metadata"] =json_encode($metadata);
             $dataji['title']=$message;
            $dataji['date']=date("d/m/y H:i");
            $dataji['source']=$company_symbol;
            $dataji['m_companies']=$company_name;
            $dataji['m_countries']=$country;
            $dataji['image_url']=base_url().$image;
            $dataji["created"] = date("Y-m-d");
            
            $dataji['link'] = "http://localhost:3000/company/$company_id/";
            //print_r($dataji);die();
            $this->db->insert("api_data", $dataji);
  ...
  public function adddocument()
  {
    ...
    if (!empty($data)) {
      ...
      if (!empty($_FILES['document']['name'])) {
        ...
        unset($data["document_company_id"]);
        unset($data["document_year"]);
        unset($data["document_period"]);
        ...
        $dataji['source']=$company_symbol;
          ...
          $dataji['m_countries']=$country;
           ...
           $dataji["type"] = "Financial Report";
            $dataji["created"] = date("Y-m-d");
            //print_r($dataji);die();
            $dataji['link'] = base_url() . "uploads/documents/$country/$company_name/$year/$period/$filename";
           $this->db->insert("api_data",$dataji);
  ...
  public function financial_news()
  {
    ...
    if ($session_id) {
      ...
      if ($this->user_type == "admin") {
        ...
        //$document = $this->db->query("SELECT * FROM `documents`")->result();
