...
public function forum_get($token=""){
    ...
    foreach($forums as $index => $value){
        ...
        $value->voted = (isset($user_data)&&$this->db->get_where("polling_select", ["forum_id"=>($value->id), "user_id"=>($user_data->id)])->row())?true:false;
        ...
        $value->liked = (isset($user_data)&&$this->db->get_where("forums_likes", ["forum_id"=>$value->id, "user_id"=>($user_data->id)])->row())?true:false;
        $value->total_liked = $this->db->select("count(*) as total")->get_where("forums_likes", ["forum_id"=>($value->id)])->row()->total;
        $value->disliked = (isset($user_data)&&$this->db->get_where("forum_dislikes", ["forum_id"=>$value->id, "user_id"=>($user_data->id)])->row())?true:false;
        ...
        $value->isMine = (isset($user_data) && ($user_data->id==$value->user_id))?true:false;
...
public function financialapinew_get($page=0, $countryname="", $companyname=""){
  ini_set('memory_limit', '-1');
  ini_set('display_errors', 0);
   $limit= 50;
   $start = $limit*$page;
...
public function getdatabyric_get($ric_name="", $time_stamps=5){
  ...
  public function subscription_post(){
    ...
    if(isset($token)&&!empty($token)&&isset($expire_on)&&!empty($expire_on)&&isset($subscription_type)&&!empty($subscription_type)&&isset($amount)){
        ...
        if($subscription_type!="Free"&&$subscription_type!="Monthly"&&$subscription_type!="Yearly"&&$subscription_type!="Quarterly"){
          $data["status"]=false;
 ...
 public function selectedcompanies_get($token=""){
  ...
  public function findancialload_post(){   //For admin only 
   		 $draw = $_POST['draw'];
  ...
  public function usereventsdtbl_post(){ //For admin side only
